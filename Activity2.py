    ...
    try:
        with open(filename, mode='r') as file:
            ...
            for contact in contacts:
                ...
                contact.setdefault('Municipality', '')
...
def save_contacts(filename, contacts):
    with open(filename, mode='w', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=["Name", "Phone", "Email", "Municipality", "Social Media"])
        writer.writeheader()
        writer.writerows(contacts)

# Function to display contacts
def display_contacts(contacts):
    print("\n{:<20} {:<15} {:<30} {:<20} {:<20}".format("Name", "Phone", "Email", "Municipality", "Social Media"))
    print("-" * 105)
    for contact in contacts:
...
def update_contact(contacts):
    ...
    for contact in contacts:
        if contact['Name'].lower() == name.lower():
            ...
            contact['Municipality'] = input(f"Enter new municipality for {name}: ")
